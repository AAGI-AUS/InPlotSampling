...
- pop:
  ...
  2.  Parent auxiliary parameter: an auxiliary parameter for ranking
      parents

  3.  Child auxiliary parameter: an auxiliary parameter for ranking
      children
...
- replace:
  ...
  A boolean which specifies whether to sample with replacement or not in
  the first stage (applicable only for JPS sampling).

- ni:

  Number(s) of samples in the second stage. Can be a single number or a
  vector of `n` numbers.

- Hi:

  Set size for each ranking group in the second stage. Can be a single
  number or a vector of `n` numbers.

- replace_i:

  A boolean which specifies whether to sample with replacement or not in
  the second stage (applicable only for JPS sampling).
